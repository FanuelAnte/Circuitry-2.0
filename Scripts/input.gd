...
func _process(delta: float) -> void:
	...
	for child in get_children():
		if child.is_in_group("toggle"):
			child.text = str(output_values[child.get_index()])
			
	for i in get_children():
		var index: int = 0
		if i.is_in_group("toggle"):
			index = i.get_index()
			i.button_pressed = output_values[index]

func set_slot_value(value: bool, slot_index: int) -> void:
	if value == true:
		output_values[slot_index] = 1
	else:
		output_values[slot_index] = 0
